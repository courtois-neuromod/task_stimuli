...
Press the button when you see an unrecognizable object that was generated."""
    ...
    def _run(self, exp_win, ctl_win):
        ...
        for trial_n, trial in enumerate(self.trials):
            ...
            self.progress_bar.set_description(f"Trial {trial_n}:: {trial['condition']}:{trial['image_path']}" )
            ...
            if ctl_win:
                self.fixation_cross.draw(ctl_win)
            while self.task_timer.getTime() < trial['onset'] + trial['duration'] - 1/config.FRAME_RATE:
                time.sleep(.0005) #just to avoid looping to fast
            yield True #flip
            trial['offset_flip'] = self._exp_win_last_flip_time - self._exp_win_first_flip_time

            while self.task_timer.getTime() < trial['onset'] + RESPONSE_TIME - 1/config.FRAME_RATE:
                time.sleep(.0005) #just to avoid looping to fast
