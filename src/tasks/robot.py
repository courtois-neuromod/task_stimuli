...
# ----------------------------------------------------------------- #
class CozmoFriends(CozmoFirstTaskPsychoPyNUC):
    def __init__(
        self,
        run,
        nuc_addr,
        ...
        **kwargs,
        ...
        super().__init__(
            nuc_addr=nuc_addr,
            tcp_port_send=tcp_port_send,
            tcp_port_recv=tcp_port_recv,
        ...
        self.wrong_place_text_time = None
    ...
    def _setup(self, exp_win):
        x_pos = exp_win.size[0] / 2
        y_pos = exp_win.size[1] * 0.9
        ...
        self.target_img_pos = (exp_win.size[0] * 0.8, exp_win.size[1] * 0.1)
        self.target_img_size = (exp_win.size[1] / 6, exp_win.size[1] / 3)
        super()._setup(exp_win)
    ...
    def _run(self, exp_win, *args, **kwargs):
        ...
        self.thread_send.start()
        i_target = 0
        target_name = self.target_names[i_target]
        ...
        while not self.done:
            ...
            if "find" in actions_list:
                ...
                if current_cell in self.target_positions[i_target]:
                    ...
                    if self.wrong_place_text_time is None:
                        self.wrong_place_text_time = (
                            self.task_timer.getTime() + 10 * 1000
                        )
            ...
            if self.max_duration and self.task_timer.getTime() > self.max_duration:
                print("timeout !")
                self.done = True
        # TODO: show congrats or timeout instructions according to self.all_founds
    ...
    def recv_loop(self):
        ...
        while not self.done:
            ...
            if len(received) > 0 and not self.done:
                ...
                self.frame_timestamp_task_timestamp_pos_pycozmo.append(
                    (id, timestamp, self.task_timer.getTime(), x_pos[0], y_pos[0])
                )
                self.robot_pos = (x_pos[0], y_pos[0])
                offset += 2 * onset
