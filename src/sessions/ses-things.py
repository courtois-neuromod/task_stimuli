...
def generate_design_file(subject, session):
    ...
    images_exp = images_list[images_list.condition.eq('exp') & images_list.exemplar_nr.eq(int(session))].sample(frac=1)
    ...
    seed = int(hashlib.sha1(("%s-%s"%(subject, session)).encode('utf-8')).hexdigest(), 16) % (2**32 - 1)
    print('seed', seed)
    np.random.seed(seed)
